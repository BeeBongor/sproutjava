...
//    });
//    System.ouT43495t.println("firstCost:" + (System.currentTimeMillis() - now));

        AtomicInteger count = new AtomicInteger(0);
//    result.stream()T43439
...
//          couhttps://www.jooq.org/doc/latest/manual/sql-execution/fetching/data-type-conversion/https://www.jooq.org/doc/3.9/manual/code-generation/custom-data-types/nt.incrementAndGet();
//        });
...
//    now = System.currentTimeMillis();
        ...
        fromCityToCities.get(fromCity).forEach(currentCity -> {
            getTheRestCitiesByNeedFlyTimesV2(fromCityToCities, currentCity, flyTimes - 1, flyTimes - 1, Lists.newArrayList(fromCity, currentCity), assignedCity, result2, true, mustViaSomeCities, false, true);
        });
...
//        })
                .filter(flyRoad -> CollectionUtils.isEmpty(mustViaSomeCities) ? true : mustViaSomeCities.stream().allMatch(flyRoad::contains)).forEach(road -> {
//          mustViaSomeCities.forEach(needNotShowCity -> road.replaceAll(needNotShowCity, "").replaceAll("->", ""));
    ...
    private void getTheRestCitiesByNeedFlyTimesV2(Map<String, Set<String>> fromCityToCities, String currentCity, int theRestFlyTimes, int totalFlyTimes, List<String> currentFlyRoads, String theAssignedCity, List<List<String>> actualSuccessRoads, boolean containsMidway, Set<String> mustViaSomeCities, boolean needCheckMustPassCity, boolean canNotPassRepeatCity) {
...
//        System.out.println(currentFlyRoad);
        ...
        if (CollectionUtils.isNotEmpty(fromCurrentCityCanReachCities)) {
            fromCurrentCityCanReachCities.forEach(reachCity -> {
                ...
                getTheRestCitiesByNeedFlyTimesV2(fromCityToCities, reachCity, theRestFlyTimes - 1, totalFlyTimes, tempRoad, theAssignedCity, actualSuccessRoads, containsMidway, mustViaSomeCities, needCheckMustPassCity, canNotPassRepeatCity);
            });
        }
    }
}
